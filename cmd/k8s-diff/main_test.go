...
// TestCLI_IdenticalManifests tests comparing identical manifests
func TestCLI_IdenticalManifests(t *testing.T) {
	// Build the binary first
	buildCmd := exec.Command("go", "build", "-o", "../../bin/k8s-diff-test", ".")
...
// TestCLI_DifferentManifests tests comparing different manifests
func TestCLI_DifferentManifests(t *testing.T) {
	buildCmd := exec.Command("go", "build", "-o", "../../bin/k8s-diff-test", ".")
	if err := buildCmd.Run(); err != nil {
		t.Fatalf("Failed to build binary: %v", err)
	}
...
// TestCLI_JSONOutput tests JSON output format
...
// TestCLI_ShowIdentical tests --show-identical flag
func TestCLI_ShowIdentical(t *testing.T) {
	buildCmd := exec.Command("go", "build", "-o", "../../bin/k8s-diff-test", ".")
	if err := buildCmd.Run(); err != nil {
		t.Fatalf("Failed to build binary: %v", err)
	}
...
// TestCLI_InvalidYAML tests error handling for invalid YAML
...
// TestCLI_MissingFile tests error handling for missing files
...
func TestCLI_Version(t *testing.T) {
	...
	if !bytes.Contains([]byte(output), []byte("k8s-diff")) {
		t.Errorf("Expected 'k8s-diff' in version output, got: %s", output)
	}
}
